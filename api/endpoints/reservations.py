"""
Routes de gestion des réservations de salles (création, consultation, modification, suppression).
...
from db.requests.reservation import (
    ...
    get_reservations_by_prof_increase,
    get_reservations_by_salle_increase,
    post_reservation,
    remove_reservation,
    remove_reservation_by_id,
    update_reservation,
)
from db.requests.user import get_user_by_id
from models.schemas import ReservationCreate, ReservationDelete, ReservationResponse, ReservationUpdate
...
@router.post(
    ...
    responses={
        201: {
            ...
            "content": {
                "application/json": {"example": {"message": "Réservation enregistrée par egauthier.", "id": 42}}
            },
        },
        400: {
            "description": "Erreur lors de la création de la réservation",
            "content": {"application/json": {"example": {"detail": "Cette salle est déjà réservée pour cet horaire"}}},
        },
        404: {
            "description": "Utilisateur non trouvé",
            "content": {"application/json": {"example": {"detail": "Utilisateur non trouvé"}}},
        },
    },
)
def create_reservation(reservation: ReservationCreate, user_id: int = Depends(verify_token)):
    ...
    if result.get("status") == "success":
        return {"message": f"Réservation enregistrée par {username}.", "id": result.get("id_reservation")}
    ...
    error_message = result.get("message") or error_mapping.get(
        result.get("status"), "Erreur lors de la création de la réservation, veuillez consulter un administrateur."
    )
...
@router.get(
    ...
    responses={
        ...
        410: {
            ...
            "content": {"application/json": {"example": {"detail": "Aucune réservation trouvée"}}},
        },
    },
)
...
@router.delete(
    ...
    responses={
        200: {
            ...
            "content": {"application/json": {"example": {"message": "Réservation supprimée avec succès"}}},
        },
        400: {
            "description": "Erreur lors de la suppression",
            "content": {"application/json": {"example": {"detail": "Erreur lors de la suppression"}}},
        },
        422: {
...
def delete_reservation(reservation: ReservationDelete, user_id: int = Depends(verify_token)):
    if reservation.id_reservation is not None:
        ...
        result = remove_reservation(
            user_id, reservation.date.isoformat(), reservation.numero_salle, reservation.heure_debut
        )
    else:
        raise HTTPException(
            status_code=422,
...
@router.put(
    ...
    responses={
        200: {
            ...
            "content": {"application/json": {"example": {"message": "Réservation modifiée avec succès"}}},
        },
        400: {
            "description": "Erreur lors de la mise à jour",
            "content": {"application/json": {"example": {"detail": "Erreur lors de la mise à jour"}}},
        },
    },
)

"""
Opérations de base de données pour la gestion des réservations.
...
def get_reservation_by_id(id_reservation: int) -> dict[str, Any] | None:
...
def post_reservation(duree, date, info, numero_salle, nom_matiere, heure_debut_creneau, login_user, nom_classe):
    with get_db_cursor() as cursor:
        try:
            ...
            cursor.execute(
                "SELECT TIME_TO_SEC(heure_debut) as seconds FROM creneau WHERE heure_debut = %s", (heure_debut_creneau,)
            )
